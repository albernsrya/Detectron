    ...
    for data_set in sets:
        ...
        with open(file_list % data_set) as f:
            for img_id, img_name in enumerate(f):
                ...
                image = {}
                mat_file = os.path.join(data_dir,
                                        "annotations/%s.mat" % img_name)
                ...
                if len(categories) == 0:
                    ...
                    for idx, n in enumerate(labelNames):
                        categories.append({
                            ...
                            idx,
                            "name":
                            "".join(chr(i) for i in data[n[0]])
                        })
                    ann_dict["categories"] = categories
                scipy.misc.imsave(os.path.join(data_dir, img_name + ".png"),
                                  labelMap)
