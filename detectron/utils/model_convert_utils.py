...
def fuse_first_affine(net, params, removed_tensors):
    ...
    for ((i, current), (j, next_)) in pairwise(enumerate(net.op)):
        ...
        if (current.type not in ("Conv", "ConvTranspose")
            ...
            continue
        if (current.output[0] != next_.output[0]
                and len(blob_uses(net, current.output[0])) != 1):
        ...
        A_ = A.reshape(-1, 1, 1, 1) if conv.type == "Conv" else A.reshape(
            1, -1, 1, 1)
...
def fuse_affine(net, params, ignore_failure):
    ...
    removed_tensors = []
    while True:
        (next_net, next_params,
         removed_tensors) = fuse_first_affine(net, params, removed_tensors)
        if len(next_net.op) == len(net.op):
            if (any(op.type == "AffineChannel" for op in next_net.op)
                    and not ignore_failure):
                raise Exception(
                    "Model contains AffineChannel op after fusion: %s",
                    next_net)
...
def compare_model(model1_func, model2_func, test_image, check_blobs):
    ...
    for idx in range(len(cb1)):
        ...
        assert r1 is not None or r2 is None, "Blob {} in model1 is None".format(
            n1)
        assert r2 is not None or r1 is None, "Blob {} in model2 is None".format(
            n2)
