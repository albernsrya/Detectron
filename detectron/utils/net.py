...
from detectron.core.config import cfg, load_cfg
...
def configure_bbox_reg_weights(model, saved_cfg):
    ...
    if "MODEL" not in saved_cfg or "BBOX_REG_WEIGHTS" not in saved_cfg.MODEL:
        ...
        cfg.immutable(False)
        cfg.MODEL.BBOX_REG_WEIGHTS = (1.0, 1.0, 1.0, 1.0)
...
def get_group_gn(dim):
    ...
    assert dim_per_gp == -1 or num_groups == - \
        1, "GroupNorm: can only specify G or C/G."
