...
class TestRoIDataLoader(unittest.TestCase):
    @mock.patch("detectron.roi_data.loader.get_minibatch_blob_names",
                return_value=["data"])
    @mock.patch("detectron.roi_data.loader.get_minibatch",
                side_effect=get_roidb_blobs)
    def test_two_parallel_loaders(self, _1, _2):
        train_data = np.random.rand(2, 3, 3).astype(np.float32)
        train_loader, train_net = create_loader_and_network(
            train_data, "dequeue_net_train")
        test_data = np.random.rand(2, 4, 4).astype(np.float32)
        test_loader, test_net = create_loader_and_network(
            test_data, "dequeue_net_test")
        for _ in range(5):
