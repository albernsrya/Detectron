...
from detectron.ops.collect_and_distribute_fpn_rpn_proposals import \
...
class DetectionModelHelper(cnn.CNNModelHelper):
    ...
    def TrainableParams(self, gpu_id=-1):
        ...
        return [
            ...
            if (p in self.param_to_grad
                and p not in self.do_not_update_params  # p has a gradient
                and (  # not on the blacklist
                    gpu_id == -1
                    or str(p).find(  # filter for gpu assignment, if gpu_id set
                        "gpu_{}".format(gpu_id)) == 0))
        ]
    ...
    def RoIFeatureTransform(
        ...
        if isinstance(blobs_in, list):
            ...
            xform_out = self.net.BatchPermutation([xform_shuffled, restore_bl],
                                                  blob_out)
        else:
    ...
    def ConvShared(self,
        ...
        return self.net.Conv(blobs_in,
                             blob_out,
                             kernel=kernel,
                             order=self.order,
                             **kwargs)

    def BilinearInterpolation(self, blob_in, blob_out, dim_in, dim_out,
                              up_scale):
        ...
        kernel = np.zeros((dim_in, dim_out, kernel_size, kernel_size),
                          dtype=np.float32)
    ...
    def ConvAffine(  # args in the same order of Conv()
        ...
        blob_out = self.AffineChannel(conv_blob,
                                      prefix + suffix,
                                      dim=dim_out,
                                      inplace=inplace)
        return blob_out
    ...
    def ConvGN(  # args in the same order of Conv()
        ...
        blob_out = self.SpatialGN(
            ...
            prefix + suffix,
            dim_out,
            group=group_gn,  # op's arg name is "group"
    ...
    def _SetNewLr(self, cur_lr, new_lr):
        ...
        ratio = _get_lr_change_ratio(cur_lr, new_lr)
        if (cfg.SOLVER.SCALE_MOMENTUM and cur_lr > 1e-7
...
def _get_lr_change_ratio(cur_lr, new_lr):
    eps = 1e-10
    ratio = np.max((new_lr / np.max((cur_lr, eps)), cur_lr / np.max(
        (new_lr, eps))))
    return ratio
