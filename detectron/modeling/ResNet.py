...
def add_ResNet_convX_body(model, block_counts):
    ...
    s, dim_in = add_stage(model, "res3", s, n2, dim_in, 512,
                          dim_bottleneck * 2, 1)
    if freeze_at == 3:
        model.StopGradient(s, s)
    s, dim_in = add_stage(model, "res4", s, n3, dim_in, 1024,
                          dim_bottleneck * 4, 1)
    if freeze_at == 4:
        model.StopGradient(s, s)
    if len(block_counts) == 4:
        ...
        if freeze_at == 5:
            model.StopGradient(s, s)
        return s, dim_in, 1.0 / 32.0 * cfg.RESNETS.RES5_DILATION
    else:
        return s, dim_in, 1.0 / 16.0
