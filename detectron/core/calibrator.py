...
class KLCalib(algorithm):
    ...
    def update_max(self, op, max_name, tensor_idx, tensor_name):
        ...
        self.remove_arg(op, name)
        max_arg = utils.MakeArgument(name,
                                     np.array([layer_max]).astype(np.float32))
        # save max vaules in predict_def as operator arguments
        op.arg.extend([max_arg])
    ...
    def get_kl_hist(self, data, min_val, max_val, name):
        hist_iter, hist_edges_iter = np.histogram(data,
                                                  bins=2048,
    ...
    def get_optimal_scaling_factor(self,
                                   hist,
                                   hist_edges,
                                   P_sum,
                                   max_val,
...
class AbsmaxCalib(algorithm):
    ...
    def get_max(self, op, blob, max_name, tensor_idx, tensor_name):
        ...
        if arg is not None:
            ...
            absmax = np.array([np.absolute([orig_absmax,
                                            absmax]).max()]).astype(np.float32)
            self.remove_arg(op, name)
...
class EMACalib(algorithm):
    ...
    def get_max(self, op, blob, max_name, tensor_idx, tensor_name):
        ...
        if arg is not None:
            orig_absmax = arg.floats[0]
            absmax = np.array([
                self.ema_alpha * absmax + (1 - self.ema_alpha) * orig_absmax
            ]).astype(np.float32)
            self.remove_arg(op, name)
...
class Calibrator(object):
    ...
    def DepositQuantizedModule(self, ws, predict_def):
        ...
        def not_need_output_scale(op_type):
            if not op_type.startswith("Int8"):
                return True
            return {"Int8Dequantize": True}.get(op_type, False)
        ...
        def predict_output_format(predict_def, op_pos):
            ...
            while op is not None:
                ...
                if fmt is not None:
                    return fmt
                op, cur_pos = self.algo.get_predecessor_op(
                    0, cur_pos, predict_def)
            return "NCHW"
        ...
        def add_order_swtich(predict_def,
                             op_pos,
                             op_type="NCHW2NHWC",
                             is_insert=True):
        ...
        def insert_quantize(predict_def):
            ...
            while cur_pos < op_len:
                ...
                while inp_index < inp_len:
                    ...
                    pre_op, pre_pos = self.algo.get_predecessor_op(
                        inp_index, new_pos, predict_def)
                    if pre_op is None:
                    ...
                    outp = inp + "_quantized_" + str(cur_pos) + "_" + str(
                        inp_index)
        ...
        def refine_module_outputs(predict_def):
            ...
            while cur_pos < op_len:
                ...
                if predict_output_format(predict_def, cur_pos) == "NHWC":
                    ...
                    deq_op = core.CreateOperator("Int8Dequantize", [deq_inp],
                                                 [deq_outp])
                    order_sw_op = core.CreateOperator("NHWC2NCHW",
                                                      [order_sw_inp],
                                                      [order_sw_outp])
        ...
        def predict_output_data_type(predict_def):
            ...
            while pos < len(predict_def.op):
                ...
                if not op.type.startswith("Int8"):
                    ...
                    _, pre_pos = self.algo.get_predecessor_op(
                        0, pos, predict_def)
                    if pre_pos is None:
                        output_data_type.append(DATA_TYPE_S8)
        ...
        def add_output_scale(predict_def, output_data_type):
            for i, op in enumerate(predict_def.op):
                ...
                if op.type == "Int8Quantize":
                    ...
                    input_index = self.algo.get_input_index(
                        op.output[0], successor)
        ...
        def quantize_weights(ws, op, init_def):
            ...
            values = (np.rint(
                (weights / output_scale)).astype(np.int8) + output_zero_point)
            ...
            for i, op in enumerate(predict_def.op):
                ...
                if has_weights(op):
                    ...
                    if has_bias(op):
                        ...
                        quantize_bias(ws, op, init_def, input_data_type,
                                      weights_scale)
            return init_def
