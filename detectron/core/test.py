...
def im_detect_bbox_scale(model,
    ...
    if hflip:
        ...
        scores_scl, boxes_scl, _ = im_detect_bbox(model,
                                                  im,
                                                  target_scale,
                                                  target_max_size,
                                                  boxes=box_proposals)
...
def im_detect_bbox_aspect_ratio(model,
    ...
    if hflip:
        ...
        scores_ar, boxes_ar, _ = im_detect_bbox(model,
                                                im_ar,
                                                cfg.TEST.SCALE,
                                                cfg.TEST.MAX_SIZE,
...
def im_detect_mask_aug(model, im, boxes):
    ...
    im_scale_i = im_conv_body_only(model, im, cfg.TEST.SCALE,
                                   cfg.TEST.MAX_SIZE)
    ...
    for scale in cfg.TEST.MASK_AUG.SCALES:
        ...
        if cfg.TEST.MASK_AUG.SCALE_H_FLIP:
            masks_scl_hf = im_detect_mask_scale(model,
                                                im,
                                                scale,
                                                max_size,
                                                boxes,
    ...
    for aspect_ratio in cfg.TEST.MASK_AUG.ASPECT_RATIOS:
        ...
        if cfg.TEST.MASK_AUG.ASPECT_RATIO_H_FLIP:
            masks_ar_hf = im_detect_mask_aspect_ratio(model,
                                                      im,
                                                      aspect_ratio,
                                                      boxes,
                                                      hflip=True)
...
def im_detect_mask_aspect_ratio(model, im, aspect_ratio, boxes, hflip=False):
    ...
    if hflip:
        ...
        im_scale = im_conv_body_only(model, im_ar, cfg.TEST.SCALE,
                                     cfg.TEST.MAX_SIZE)
...
def im_detect_keypoints_aug(model, im, boxes):
    ...
    if cfg.TEST.KPS_AUG.H_FLIP:
        heatmaps_hf = im_detect_keypoints_hflip(model, im, cfg.TEST.SCALE,
                                                cfg.TEST.MAX_SIZE, boxes)
    ...
    for scale in cfg.TEST.KPS_AUG.SCALES:
        ...
        heatmaps_scl = im_detect_keypoints_scale(model, im, scale,
                                                 cfg.TEST.KPS_AUG.MAX_SIZE,
                                                 boxes)
    ...
    for aspect_ratio in cfg.TEST.KPS_AUG.ASPECT_RATIOS:
        heatmaps_ar = im_detect_keypoints_aspect_ratio(model, im, aspect_ratio,
                                                       boxes)
        add_heatmaps_t(heatmaps_ar)

        if cfg.TEST.KPS_AUG.ASPECT_RATIO_H_FLIP:
            heatmaps_ar_hf = im_detect_keypoints_aspect_ratio(model,
                                                              im,
                                                              aspect_ratio,
                                                              boxes,
                                                              hflip=True)
    ...
    if cfg.TEST.KPS_AUG.SCALE_SIZE_DEP:
        heatmaps_c = combine_heatmaps_size_dep(heatmaps_ts, ds_ts, us_ts,
                                               boxes, heur_f)
    else:
...
def im_detect_keypoints_scale(model,
                              im,
                              target_scale,
                              target_max_size,
                              boxes,
    ...
    if hflip:
        heatmaps_scl = im_detect_keypoints_hflip(model, im, target_scale,
                                                 target_max_size, boxes)
    else:
        im_scale = im_conv_body_only(model, im, target_scale, target_max_size)
...
def im_detect_keypoints_aspect_ratio(model,
                                     im,
                                     aspect_ratio,
                                     boxes,
                                     hflip=False):
    ...
    if hflip:
        heatmaps_ar = im_detect_keypoints_hflip(model, im_ar, cfg.TEST.SCALE,
                                                cfg.TEST.MAX_SIZE, boxes_ar)
    else:
        im_scale = im_conv_body_only(model, im_ar, cfg.TEST.SCALE,
                                     cfg.TEST.MAX_SIZE)
