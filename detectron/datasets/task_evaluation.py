...
def evaluate_all(dataset,
    ...
    """
    all_results = evaluate_boxes(dataset,
                                 all_boxes,
                                 output_dir,
...
def evaluate_keypoints(dataset, all_boxes, all_keyps, output_dir):
    ...
    assert dataset.name.startswith(
        "keypoints_coco_"), "Only COCO keypoints are currently supported"
...
def evaluate_box_proposals(dataset, roidb):
    ...
    for limit in [100, 1000]:
        for area, suffix in areas.items():
            stats = json_dataset_evaluator.evaluate_box_proposals(dataset,
                                                                  roidb,
                                                                  area=area,
                                                                  limit=limit)
...
def check_expected_results(results, atol=0.005, rtol=0.1):
    ...
    for dataset, task, metric, expected_val in cfg.EXPECTED_RESULTS:
        ...
        assert metric in results[dataset][
            task], "Metric {} not in results".format(metric)
        ...
        msg = ("{} > {} > {} sanity check (actual vs. expected): "
               "{:.3f} vs. {:.3f}, err={:.3f}, tol={:.3f}").format(
                   dataset, task, metric, actual_val, expected_val, err, tol)
        if err > tol:
            msg = "FAIL: " + msg
            logger.error(msg)
            if cfg.EXPECTED_RESULTS_EMAIL != "":
                subject = "Detectron end-to-end test failure"
                job_name = (os.environ["DETECTRON_JOB_NAME"]
                            if "DETECTRON_JOB_NAME" in os.environ else
                            "<unknown>")
                job_id = (os.environ["WORKFLOW_RUN_ID"]
