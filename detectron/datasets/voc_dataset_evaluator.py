...
def _write_voc_results_files(json_dataset, all_boxes, salt):
    ...
    assert os.path.exists(
        image_set_path), "Image set path does not exist: {}".format(
            image_set_path)
        ...
        with open(filename, "wt") as f:
            for im_ind, index in enumerate(image_index):
                ...
                if type(dets) == list:
                    assert len(
                        dets
                    ) == 0, "dets should be numpy.ndarray or empty list"
                    continue
...
def _do_matlab_eval(json_dataset, salt, output_dir="output"):
    ...
    logger.info("-----------------------------------------------------")
    info = voc_info(json_dataset)
    path = os.path.join(cfg.ROOT_DIR, "detectron", "datasets",
...
def voc_info(json_dataset):
    ...
    assert os.path.exists(devkit_path), "Devkit directory {} not found".format(
        devkit_path)
    anno_path = os.path.join(devkit_path, "VOC" + year, "Annotations",
                             "{:s}.xml")
    image_set_path = os.path.join(devkit_path, "VOC" + year, "ImageSets",
                                  "Main", image_set + ".txt")
    return dict(
        year=year,
        image_set=image_set,
